...
// based on the passed block height (or block time for DDACOIN) to the provided
// address.  When the address is nil, the coinbase transaction will instead be
// redeemable by anyone.  For DDACOIN (time-based consensus), blockTime must be
// set; for other nets it is ignored.
//
// See the comment for NewBlockTemplate for more information about why the nil
// address handling is useful.
func createCoinbaseTx(params *chaincfg.Params, coinbaseScript []byte, nextBlockHeight int32, blockTime time.Time, addr btcutil.Address) (*btcutil.Tx, error) {
...
func (g *BlkTmplGenerator) NewBlockTemplate(payToAddress btcutil.Address) (*BlockTemplate, error) {
	...
	// Block timestamp (used for DDACOIN time-based subsidy and for block header).
	blockTime := medianAdjustedTime(best, g.timeSource)
	...
	coinbaseTx, err := createCoinbaseTx(g.chainParams, coinbaseScript,
		nextBlockHeight, blockTime, payToAddress)
	if err != nil {
		return nil, err
	}
...
mempoolLoop:
	...
	// Use the block time computed earlier for DDACOIN coinbase; for difficulty use same.
	reqDifficulty, err := g.chain.CalcNextRequiredDifficulty(blockTime)
	if err != nil {
		return nil, err
	}
